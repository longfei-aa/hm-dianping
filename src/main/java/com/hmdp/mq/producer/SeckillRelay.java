...
public class SeckillRelay implements InitializingBean {
    ...
    public void afterPropertiesSet() {
        try {
            // 不存在则创建消费组（MKSTREAM）
            redisStringTemplate.opsForStream().createGroup(STREAM_KEY, ReadOffset.latest(), GROUP);
    ...
    private void readAndHandle(ReadOffset offset) {
        List<MapRecord<String, Object, Object>> msgs = redisStringTemplate.opsForStream().read(
        ...
        for (MapRecord<String, Object, Object> m : msgs) {
            ...
            try {
                ...
                // 成功再 ACK
                redisStringTemplate.opsForStream().acknowledge(STREAM_KEY, GROUP, m.getId());
            } catch (Exception e) {
